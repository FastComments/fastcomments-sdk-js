# CommentLogData
## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
| **clearContent** | **Boolean** |  | [optional] [default to null] |
| **isDeletedUser** | **Boolean** |  | [optional] [default to null] |
| **phrase** | **String** |  | [optional] [default to null] |
| **badWord** | **String** |  | [optional] [default to null] |
| **word** | **String** |  | [optional] [default to null] |
| **locale** | **String** |  | [optional] [default to null] |
| **tenantBadgeId** | **String** |  | [optional] [default to null] |
| **badgeId** | **String** |  | [optional] [default to null] |
| **wasLoggedIn** | **Boolean** |  | [optional] [default to null] |
| **foundUser** | **Boolean** |  | [optional] [default to null] |
| **verified** | **Boolean** |  | [optional] [default to null] |
| **engine** | **String** |  | [optional] [default to null] |
| **engineResponse** | **String** |  | [optional] [default to null] |
| **engineTokens** | **Double** |  | [optional] [default to null] |
| **trustFactor** | **Double** |  | [optional] [default to null] |
| **rule** | [**SpamRule**](SpamRule.md) |  | [optional] [default to null] |
| **userId** | **String** |  | [optional] [default to null] |
| **subscribers** | **Double** |  | [optional] [default to null] |
| **notificationCount** | **Double** |  | [optional] [default to null] |
| **votesBefore** | **Double** |  | [optional] [default to null] |
| **votesUpBefore** | **Double** |  | [optional] [default to null] |
| **votesDownBefore** | **Double** |  | [optional] [default to null] |
| **votesAfter** | **Double** |  | [optional] [default to null] |
| **votesUpAfter** | **Double** |  | [optional] [default to null] |
| **votesDownAfter** | **Double** |  | [optional] [default to null] |
| **repeatAction** | [**RepeatCommentHandlingAction**](RepeatCommentHandlingAction.md) |  | [optional] [default to null] |
| **reason** | [**RepeatCommentCheckIgnoredReason**](RepeatCommentCheckIgnoredReason.md) |  | [optional] [default to null] |
| **otherData** | [**oas_any_type_not_mapped**](.md) |  | [optional] [default to null] |
| **spamBefore** | **Boolean** |  | [optional] [default to null] |
| **spamAfter** | **Boolean** |  | [optional] [default to null] |
| **permanentFlag** | **String** |  | [optional] [default to null] |
| **approvedBefore** | **Boolean** |  | [optional] [default to null] |
| **approvedAfter** | **Boolean** |  | [optional] [default to null] |
| **reviewedBefore** | **Boolean** |  | [optional] [default to null] |
| **reviewedAfter** | **Boolean** |  | [optional] [default to null] |
| **textBefore** | **String** |  | [optional] [default to null] |
| **textAfter** | **String** |  | [optional] [default to null] |
| **expireBefore** | **Date** |  | [optional] [default to null] |
| **expireAfter** | **Date** |  | [optional] [default to null] |
| **flagCountBefore** | **Double** |  | [optional] [default to null] |
| **trustFactorBefore** | **Double** |  | [optional] [default to null] |
| **trustFactorAfter** | **Double** |  | [optional] [default to null] |
| **referencedCommentId** | **String** |  | [optional] [default to null] |

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)

